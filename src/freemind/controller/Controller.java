...
public class Controller {
	...
	public Controller(FreeMindMain frame) {
		...
		if (!Tools.isAvailableFontFamily(getProperty("standardfont"))) {
			System.out.println("Warning: the font you have set as standard - "
					+ getProperty("standardfont") + " - is not available.");
	...
	public boolean changeToMode(String mode) {
		...
		if (getMode().getLeftToolBar() != null) {
			getFrame().getContentPane().add(getMode().getLeftToolBar(),
					BorderLayout.WEST);
	...
	public void informationMessage(Object message) {
		JOptionPane.showMessageDialog(getFrame().getContentPane(), message
				.toString(), "FreeMind", JOptionPane.INFORMATION_MESSAGE);
	}
	...
	public void setZoom(float zoom) {
		...
		MessageFormat formatter = new MessageFormat(
				getResourceString("user_defined_zoom_status_bar"));
		getFrame().out(formatter.format(messageArguments));
	}
	...
	private void quit() {
		...
		if (currentMapRestorable != null) {
			getFrame().setProperty("onStartIfNotSpecified",
		...
		setProperty("appwindow_height", String.valueOf(getFrame()
				.getWinHeight()));
		...
		void nextMap() {
			if (false) {
				if (current + 1 < historyList.size()) {
					getMapModuleManager().setMapModule(
							(MapModule) historyList.get(++current));
	...
	private class PrintAction extends AbstractAction {
		...
		PrintAction(Controller controller, boolean isDlg) {
			super(controller.getResourceString("print"), new ImageIcon(
	...
	private class PageAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			final JCheckBox fitToPage = new JCheckBox(
					getResourceString("fit_to_page"), Tools.safeEquals("true",
							getProperty("fit_to_page")));
	...
	private class DocumentationAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// if (map.startsWith(".")) {
			// map = "file:"+System.getProperty("user.dir") +
			// map.substring(1);//remove "." and make url
			// }
			// ((BrowseController)getMode().getModeController()).loadURL(map);
			// //IMPROVE THIS!
			// // } catch (FileNotFoundException ex) {
			// // JOptionPane.showMessageDialog(getView(),
			// getResourceString("file_not_found") +
	...
	private class LicenseAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			JOptionPane.showMessageDialog(getView(), controller
					.getResourceString("license_text"));
		}
	}
	...
	private class HistoryNextMapAction extends AbstractAction {
		HistoryNextMapAction(Controller controller) {
			super(controller.getResourceString("next_map"), new ImageIcon(
					getResource("images/Forward24.gif")));
			setEnabled(false);
		}
	...
	public class PropertyAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			final OptionPanel options = new OptionPanel(getFrame(), dialog,
					new OptionPanelFeedback() {
						...
						public void writeProperties(Properties props) {
							...
							for (Iterator i = Controller
									.getPropertyChangeListeners().iterator(); i
									.hasNext();) {
								FreemindPropertyListener listener = (FreemindPropertyListener) i
										.next();
								for (Iterator j = oldProperties.keySet()
										.iterator(); j.hasNext();) {
									String key = (String) j.next();
									listener.propertyChanged(key, controller
											.getProperty(key),
											(String) oldProperties.get(key));
								}
							}
	...
	/**
	 * Seems to be obsolete, but we add the property listener here. fc,
	 * 14.6.2005
	 */
	private class BackgroundAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			Color color = showCommonJColorChooserDialog(getView(),
					getResourceString("choose_background_color"), getView()
							.getBackground());
	...
	private class OpenFAQAction extends AbstractAction {
		...
		OpenFAQAction(Controller controller) {
			super(controller.getResourceString("FAQ"), new ImageIcon(controller
		...
		public void actionPerformed(ActionEvent e) {
			try {
				c.getFrame().openDocument(
						new URL("http://freemind.sourceforge.net/faq.html"));
	...
	public WindowConfigurationStorage decorateDialog(JDialog dialog,
		...
		if (unmarshalled != null) {
			...
			if (storage != null) {
				...
				dialog.getRootPane().setPreferredSize(
	...
	public void storeDialogPositions(JDialog dialog,
			WindowConfigurationStorage storage, String propertyName) {
	...
	public XmlAction unMarshall(String inputString) {
		try {
			...
			XmlAction doAction = (XmlAction) u.unmarshal(new StreamSource(
