 ...
 */
/*$Id: IconAction.java,v 1.1.4.2 2005/05/03 05:29:50 christianfoltin Exp $*/
...
public class IconAction extends FreemindAction implements ActorXml {
	...
	public IconAction(ModeController controller, MindIcon _icon,
			RemoveLastIconAction removeLastIconAction) {
		super(_icon.getDescription(controller.getFrame()), _icon
				.getIcon(controller.getFrame()), controller);
		this.modeController = controller;
		this.removeLastIconAction = removeLastIconAction;
		putValue(Action.SHORT_DESCRIPTION, _icon.getDescription(controller
				.getFrame()));
		this.icon = _icon;
	...
	private ActionPair getActionPair(MindMapNode node, MindIcon icon)
		...
		XmlAction undoAction = removeLastIconAction
				.createRemoveLastIconXmlAction(node);
	...
	public AddIconAction createAddIconAction(MindMapNode node, MindIcon icon)
			throws JAXBException {
		AddIconAction action = modeController.getActionXmlFactory()
				.createAddIconAction();
