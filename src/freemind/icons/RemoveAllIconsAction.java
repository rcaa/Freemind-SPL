 ...
 */
/*$Id: RemoveAllIconsAction.java,v 1.1.4.1 2004/10/17 23:00:10 dpolivaev Exp $*/
...
public class RemoveAllIconsAction extends NodeGeneralAction implements
	 ...
	 */
	public RemoveAllIconsAction(ModeController modeController,
	...
	public ActionPair apply(MapAdapter model, MindMapNode selected)
		...
		CompoundAction undoAction = modeController.getActionXmlFactory()
				.createCompoundAction();
		for (Iterator i = selected.getIcons().iterator(); i.hasNext();) {
			MindIcon icon = (MindIcon) i.next();
			undoAction.getCompoundActionOrSelectNodeActionOrCutNodeAction()
					.add(addIconAction.createAddIconAction(selected, icon));
		}
	...
	public RemoveAllIconsXmlAction createRemoveAllIconsXmlAction(
			MindMapNode node) throws JAXBException {
		RemoveAllIconsXmlAction action = modeController.getActionXmlFactory()
