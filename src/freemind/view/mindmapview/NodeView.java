...
import freemind.main.Tools; //ICON AOP
...
public abstract class NodeView extends JLabel {
	...
	// ICON publico para acesso do aspecto
	public MapView map;
	protected EdgeView edge;
	/** the Color of the Rectangle of a selected Node */
	protected final static Color selectedColor = new Color(210, 210, 210); // Color
	// .
	// lightGray
	// ;
	// //the
	...
	protected NodeView(MindMapNode model, MapView map) {
		...
		if (standardNodeColor == null) {
			standardNodeColor = Tools.xmlToColor(map.getController()
					.getProperty(FreeMind.RESOURCES_NODE_COLOR));
			// add listener:
			Controller
		...
		if (standardSelectColor == null) {
			String stdcolor = map.getController().getFrame()
					.getProperty(FreeMind.RESOURCES_SELECTED_NODE_COLOR);
	...
	public boolean isInVerticalRegion(double xCoord, double p) {
		return isLeft() ? xCoord > getSize().width * (1.0 - p)
	...
	public boolean isInFollowLinkRegion(double xCoord) {
		return getModel().getLink() != null
				&& (getModel().isRoot() || !getModel().hasChildren() || isInVerticalRegion(
						xCoord, 1. / 2));
	}
	...
	private void getCoordinates(LinkedList inList,
		...
		additionalDistanceForConvexHull = coordinatesCloudHook(
				additionalDistanceForConvexHull, byChildren);
		inList.addLast(new Point(-additionalDistanceForConvexHull
				+ getExtendedX(), -additionalDistanceForConvexHull
				+ getExtendedY()));
		inList.addLast(new Point(-additionalDistanceForConvexHull
				+ getExtendedX(), additionalDistanceForConvexHull
				+ getExtendedY() + getExtendedHeight()));
		inList.addLast(new Point(additionalDistanceForConvexHull
				+ getExtendedX() + getExtendedWidth(),
				additionalDistanceForConvexHull + getExtendedY()
						+ getExtendedHeight()));
		inList.addLast(new Point(additionalDistanceForConvexHull
				+ getExtendedX() + getExtendedWidth(),
				-additionalDistanceForConvexHull + getExtendedY()));
	...
	private int coordinatesCloudHook(int additionalDistanceForConvexHull,
			boolean byChildren) {
	...
	private static boolean NEED_PREF_SIZE_BUG_FIX = Controller.JAVA_VERSION
			.compareTo("1.5.0") < 0;
	...
	public Dimension getPreferredSize() {
		...
		prefSize.width = Math.max(map.getZoomed(MIN_HOR_NODE_SIZE),
				prefSize.width);
		if (isEmpty) {
			setText("");
		}
	...
	public void requestFocus() {
		map.getController().getMode().getModeController()
				.anotherNodeSelected(getModel());
		super.requestFocus();
	}
	...
	protected void paintBackground(Graphics2D graphics, Dimension size,
			Color color) {
	 ...
	 *            the outpoint should point in the direction of destinationPoint
	 * @param isLeft
	 *            TODO
	 * @return
	 */
	...
	public Point getLinkPoint(Point declination) {
		...
		if (y != 0) {
			...
			return new Point(getLocation().x + size.width / 2
					+ (x > 0 ? absLinkX : -absLinkX), getLocation().y
					+ size.height / 2 + (y > 0 ? absLinkY : -absLinkY));
		} else {
	...
	void update() {
		...
		setHorizontalTextPosition((getModel().isOneLeftSideOfRoot()) ? SwingConstants.LEADING
		...
		// iconPresent = true;
		// ImageIcon icon = new
		// ImageIcon(((NodeAdapter)getModel()).getFrame().getResource
		// ("images/Folded.png"));
		// iconImages.addImage(icon);
		// }
	...
	public int getZoomedFoldingSymbolHalfWidth() {
		int preferredFoldingSymbolHalfWidth = map
				.getZoomedFoldingSymbolHalfWidth();
		return Math.min(preferredFoldingSymbolHalfWidth,
	 ...
	 * %
	 * 40ID-77081.news.dfncis.de&rnum=1&prev=/groups%3Fq%3Djava%2520komplement%
	 * 25 C3
	 * %25A4rfarbe%2520helligkeit%26hl%3Dde%26lr%3D%26ie%3DUTF-8%26sa%3DN%26
	 * as_qdr %3Dall%26tab%3Dwg
	 */
	/**
