...
public class MindMapLayout implements LayoutManager {
	...
	/**
	 * This funcion resizes the map and do the layout. All tree heights, widths
	 * and shifts should be already calculated.
	 */
	...
	/**
	 * This places the node's subtree if the relative position of every node to
	 * its parent is known.
	 */
	private void layout(NodeView node) {
		// The overall task of layout is: give me the sizes and I'll set the
		// positions.
		...
		// Iterations
		for (ListIterator e = node.getChildrenViews().listIterator(); e
				.hasNext();) {
	...
	/**
	 * Set the position and the size of the node. Set the position of the edge
	 * of the node (the edge of the node is the edge connecting the node to its
	 * parent).
	 * 
	...
	private void placeNode(NodeView node, int relativeX, int relativeY) {
		...
		// I don't undertand why I reset the size of the node according to its
		// preferred size.
		// This should not be the task of this method.

		// relativeX, relativeY - already calculated coordinates of node
		// relative to its parent.;
		if (node.isRoot()) {
			node.setBounds(getRootX(), getRootY());
		} else {
			...
			// It seems that there is a piece of coding ready for having
			// labelled edges.
			// Having labelled edges is a nice thing, as sure as hell, but we do
			// not
			...
			label.setBounds(absX - label.getPreferredSize().width / 2, absY
	...
	private int calcXBorderSize() {
		int xBorderSize;
		{
			Dimension visibleSize = map.getViewportSize();
			if (visibleSize != null) {
				xBorderSize = Math.max(visibleSize.width,
						2 * (BORDER + MINIMAL_LEAF_WIDTH));
			} else {
	...
	/**
	 * This is called by treeNodesChanged(), treeNodesRemoved() &
	 * treeNodesInserted(), so it's the standard mechanism to update the
	 * graphical node structure. It updates the parent of the significant node,
	 * and follows recursivly the hierary upwards to root.
	 */
	...
	private void updateRelativeYOfChildren(NodeView node,
		...
		while (e.hasNext()) {
			...
			// TODO CLOUDS remover getAdditionalCloudHeigth dessa classe
			int iAdditionalCloudHeigth = getAdditionalCloudHeight(child);
			int upperChildShift = child.getUpperChildShift();
			child.relYPos = pointer + upperChildShift + iAdditionalCloudHeigth
					/ 2 + shiftDown;
			upperShift += upperChildShift + shiftUp;
			pointer += child.getTreeHeight() + shiftUp + shiftDown
					+ iAdditionalCloudHeigth + vgap;
		}
	...
	private int getAdditionalCloudHeight(NodeView child) {
		// return child.getAdditionalCloudHeigth();
		return 0;
	}
	...
	protected void updateTreeGeometry(NodeView node) {
		if (node.isRoot()) {
			...
			int leftTreeHeight = calcTreeHeight(node, leftTreeShift,
					leftNodeViews);
			int rightTreeHeight = calcTreeHeight(node, rightTreeShift,
					rightNodeViews);
	...
	private int calcUpperChildShift(NodeView node, LinkedList childrenViews) {
		try {
			...
			int childShift = -firstChild.relYPos
					+ getAdditionalCloudHeight(firstChild)
	...
	private int calcStandardTreeShift(NodeView parent, LinkedList childrenViews) {
		...
		for (ListIterator e = childrenViews.listIterator(); e.hasNext();) {
			...
			if (node != null) {
				height += node.getPreferredSize().height + vgap
						+ getAdditionalCloudHeight(node);
			}
		}
		return Math.max(height - vgap - parentHeight, 0) / 2;
	}

	// public int nodeAdditionalCloudHeigth(NodeView node) {
	 ...
	 * @return
	 */
	private int calcTreeHeight(NodeView parent, int treeShift,
			LinkedList childrenViews) {
		int parentHeight = parent.getPreferredSize().height;
		try {
			NodeView firstChild = (NodeView) childrenViews.getFirst();
			...
			int minY = Math.min(
					firstChild.relYPos - firstChild.getUpperChildShift()
							- getAdditionalCloudHeight(firstChild), 0);
			int maxY = Math
					.max(lastChild.relYPos - lastChild.getUpperChildShift()
							+ lastChild.getTreeHeight()
							+ getAdditionalCloudHeight(lastChild), parentHeight);
			return maxY - minY;
		} catch (NoSuchElementException e) {
			return parentHeight;
		}
	...
	private int calcTreeWidth(NodeView parent, LinkedList childrenViews) {
		...
		return parent.getPreferredSize().width + 2
				* getAdditionalCloudHeight(parent) + treeWidth;
	}
