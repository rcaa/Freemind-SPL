...
public class EditNodeTextField extends EditNodeBase {
	...
	public void show() {
		...
		/*
		 * fc, 12.10.2003: the following method is not correct. Even more with
		 * the zoom factors!
		 */
		...
		// TODO ICON tem que remover esse metodo getIcons
		if (getIconsHook()) {
			// fc, 24.9.2003 full ok for the moment, that an icon has the same
			// size as the link icon.
			xOffset = linkIconWidthHook(xOffset, linkIconWidth);
			widthAddition -= linkIconWidth;
		}

		System.out.println("xOffset ANTES = " + xOffset);

		hookTextField(xOffset);

		System.out.println("xOffset DEPOIS = " + xOffset);
		...
		if (MINIMAL_LEAF_WIDTH > xSize
				&& (getNode().getModel().isFolded() || !getNode().getModel()
						.hasChildren())) {
		...
		// changes
		Point frameScreenLocation = getFrame().getLayeredPane()
				.getLocationOnScreen();
		Point nodeScreenLocation = getNode().getLocationOnScreen();

		int xLeft = (int) (nodeScreenLocation.getX()
				- frameScreenLocation.getX() + xOffset);
		if (xExtraWidth < 0) {
			xLeft += xExtraWidth;
		}
	...
	// ICONS
	private int linkIconWidthHook(int xOffset, int linkIconWidth) {
		// xOffset += linkIconWidth * getNode().getModel().getIcons().size();
		// return xOffset;
		return 0;
	}

	private boolean getIconsHook() {
		// getNode().getModel().getIcons().size() != 0;
		return false;
	}

	// ICONS
	private void hookTextField(int offset) {
		offset++;
	}
