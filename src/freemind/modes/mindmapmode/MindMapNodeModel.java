...
public class MindMapNodeModel extends NodeAdapter {
	...
	public int saveHTML(Writer fileout, String parentID, int lastChildNumber,
			boolean isRoot, boolean treatAsParagraph, int depth)
			throws IOException {
		...
		if (font != null && font.getSize() != 0) {
			int defaultFontSize = Integer.parseInt(getFrame().getProperty(
					"defaultfontsize"));
		...
		// fileout.write("<img src=\"" + ((MindIcon)
		// getIcons().get(i)).getIconFileName() +
		// "\" alt=\""+((MindIcon)
		// getIcons().get(i)).getDescription(getFrame())+"\">"); }}

		// ICON AOP - hook
		hookHTMLIcons(fileout);
		...
		if (this.toString().matches(" *")) {
			...
			fileout
					.write(saveHTML_escapeUnicodeAndSpecialCharacters(toString()));
		}
		...
		if (getFrame().getProperty("html_export_folding").equals(
			...
			for (ListIterator e = childrenUnfolded(); e.hasNext();) {
				...
				lastChildNumber = child.saveHTML(fileout, parentID,
		...
		if (hasChildren()) {
			if (getFrame().getProperty("html_export_folding").equals(
				...
				for (ListIterator e = childrenUnfolded(); e.hasNext();) {
					MindMapNodeModel child = (MindMapNodeModel) e.next();
					localLastChildNumber = child.saveHTML(fileout,
							localParentID, localLastChildNumber,/* isRoot= */
	...
	// ICONS AOP
	private void hookHTMLIcons(Writer fileout) throws IOException {
	}
	...
	public void saveRTF(Writer fileout, int depth, HashMap colorTable)
		...
		for (ListIterator e = childrenUnfolded(); e.hasNext();) {
			((MindMapNodeModel) e.next()).saveRTF(fileout, depth + 1,
					colorTable);
		}
	}
