...
public class ApplyPatternAction extends NodeGeneralAction implements
	...
	public void applyPattern(MindMapNode node, StylePattern pattern) {
		if (pattern.getAppliesToNode()) {
			...
			getModeController().setNodeBackgroundColor(node,
					pattern.getNodeBackgroundColor());
			// FIXME: fc, 3.1.2004: setting the style to "null" causes strange
			// behaviour.
			// see https://sourceforge.net/tracker/?func=detail&atid=107118&aid=
			// 1094623&group_id=7118
			...
			if (pattern.getAppliesToNodeFont()) {
				...
				if (nodeFontFamily == null) {
					nodeFontFamily = getModeController().getController()
	...
	// ICON AOP - hook
	private void hookApplyPattern(MindMapNode node, StylePattern pattern) {
	}
